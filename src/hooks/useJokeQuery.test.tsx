...
describe('useJokeQuery', () => {
  ...
  it('fetches a joke by index', async () => {
    const { result } = renderHook(() => useJokeQuery(index), { wrapper: Wrapper })

    await waitFor(() => expect(jokes.getJoke).toHaveBeenCalledWith(index))
    expect(result.current.joke).toEqual(jokeType)
    expect(result.current.error).toBeNull()
  })

  it('does not fetch when index is undefined', () => {
