...
export const useJokeNavigation = (initialIndex?: string): UseJokeNavigationResult => {
  const [timeline, setTimeline] = useState<string[]>(initialIndex ? [initialIndex] : [])
  const [cursor, setCursor] = useState(initialIndex ? 0 : -1)
  ...
  const index = useMemo(
    () => (cursor >= 0 && cursor < timeline.length ? timeline[cursor] : undefined),
    [cursor, timeline],
  )
  ...
  useEffect(() => {
    const init = async () => {
      isRefilling.current = true
      try {
        const jokes = await getRandomJokes(initialIndex ? [initialIndex] : [])
        jokes.forEach((j) => client.setQueryData<JokeType>(['joke', j.id], j.data))

        if (!initialIndex && jokes.length > 0) {
